...
# 生成多圈螺旋的均值
angles = [k * (2 * np.pi / COMP) for k in range(COMP * num_turns)]  # 扩展角度范围
...
# 构建目标分布 (π_1)
target_mix = Categorical(torch.tensor([1 / len(vertices_1) for _ in range(len(vertices_1))]))  # 均匀权重
target_comp = MultivariateNormal(torch.tensor(vertices_1).float(),  # 各高斯分布的均值
                                  VAR * torch.stack([torch.eye(2) for _ in range(len(vertices_1))]))  # 方差矩阵
...
plt.tight_layout()
# plt.savefig('figures/2round_spiral_dataset.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
class RectifiedFlow2():
  ...
  def new_gt(self, first_order_gt, second_order_gt, z_t, t, d, flag): 
    ...
    return first_order_gt.detach(), second_order_gt
...
def draw_plot(rectified_flow, z0, z1, N=None):
  ...
  plt.title('HOMO optimized with\n (M1 + M2 + SC) losses', fontsize=19)
  plt.tight_layout()
  # plt.savefig('../figures/2_123_2round_spiral_output.pdf', format='pdf', bbox_inches='tight') # Should change
...
rectified_flow_2 = RectifiedFlow2(modellist, num_steps=128)
# import copy
...
# draw_plot(rectified_flow_2, z0=initial_model.sample([600]), z1=target_model.sample([600]).detach().clone(), N = 128)
draw_plot(rectified_flow_2, z0=initial_model.sample([600]), z1=target_model.sample([600]).detach().clone(), N = 16)
