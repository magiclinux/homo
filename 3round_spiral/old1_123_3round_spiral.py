...
samples_0 = initial_model.sample([900])  # 从单一高斯分布中采样
...
samples_1 = target_model.sample([900])  # 从目标分布中采样
...
plt.tight_layout()
# plt.savefig('../figures/3round_spiral_dataset.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
def draw_plot(rectified_flow, z0, z1, N=None):
  ...
  plt.tight_layout()
  # plt.savefig('../figures/2_123_3round_spiral_output.pdf', format='pdf', bbox_inches='tight') # Should change
...
draw_plot(rectified_flow_2, z0=initial_model.sample([600]), z1=target_model.sample([600]).detach().clone())
