...
M = D_1 + 5  # 绘图范围
VAR = 0.3  # 方差
COMP = 8  # 组件数目（八边形的8个顶点）
device = torch.device('cpu')

# 八边形的顶点坐标 (π_0)
angles = [k * (2 * np.pi / COMP) for k in range(COMP)]  # 8个顶点的角度
...
samples_0 = initial_model.sample([800])

# 八边形的顶点坐标 (π_1)
rotation_angle = 2 * np.pi / 16  # 顺时针旋转 2π/16
...
samples_1 = target_model.sample([800])
...
plt.tight_layout()
# plt.savefig('8_dataset.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
class RectifiedFlow2(nn.Module):
...
def train_rectified_flow(rectified_flow, optimizer, pairs, batchsize, inner_iters):
  ...
  for i in tqdm(range(inner_iters+1)):
    ...
    num_ones = num_elements // 4  # the number of 1s
...
def draw_plot(rectified_flow, z0, z1, N=None):
  ...
  plt.tight_layout()
  # plt.savefig('8_123_output.pdf', format='pdf', bbox_inches='tight') # Should change 
...
z0 = initial_model.sample([800])
z1 = target_model.sample([800]).detach().clone()
draw_plot(rectified_flow_2, z0=z0, z1=z1)
z_t, t, first_order_gt, second_order_gt = rectified_flow_2.get_train_tuple(z0=z0, z1=z1)
d = torch.zeros_like(t)
thop_input = (z_t, t, d)
flops, params = profile(rectified_flow_2, thop_input)
