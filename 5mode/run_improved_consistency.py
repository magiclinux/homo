...
class ImprovedConsistencyTraining():
    ...
    def get_karras_schedule(self, n, N):
        ...
        t \in [0, 1] (注意：这里0是源，1是目标，与常见Diffusion相反，但逻辑一致)
        """
        # 简单的 polynomial schedule 模拟
        # t_i = (i / (N-1))^rho
        t = (n / (N - 1)) ** self.rho
        return t
    ...
    def pseudo_huber_loss(self, input, target, c=1.0):
        ...
        Better robustness than MSE (L2) for outliers and unstable training phases.
        Loss = sqrt(|x-y|^2 + c^2) - c
        Note: c=1.0 使损失函数更接近 MSE，更稳定
        """
    ...
    def get_consistency_loss(self, z0, z1):
        ...
        # 2. Convert to continuous time using Karras Schedule or Linear
        # 这里为了对比方便，依然使用线性，但你可以启用下面的 Karras
        # t_current = self.get_karras_schedule(n, self.N)
        # t_next    = self.get_karras_schedule(n + 1, self.N)
        t_current = n / self.N
        t_next = (n + 1) / self.N
        ...
        # 4. Student predicts from 'next' (closer to target in this flow setup)
        ...
        # 6. [Improved] Pseudo-Huber Consistency Loss
...
def draw_plot(cm_trainer, z0, z1):
    ...
    plt.tight_layout()
    plt.savefig('5_improved_cm_output.pdf', format='pdf', bbox_inches='tight')
    plt.show()
...
iterations = 2000 # 增加迭代次数以充分利用 Pseudo-Huber 的收敛特性
batchsize = 2048
input_dim = 2
...
# 使用改进的训练器
cm_trainer = ImprovedConsistencyTraining(model, num_steps=40) # 增加离散化步数
...
plt.plot(loss_curve)
plt.title('Pseudo-Huber Consistency Loss')
plt.show()
