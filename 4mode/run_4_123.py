...
M = D_1 + 5  # 绘图范围
VAR = 0.3  # 方差
COMP = 4  # 组件数目（四边形的4个顶点）
device = torch.device('cpu')

# 四边形的顶点坐标 (π_0)
angles = [k * (2 * np.pi / COMP) for k in range(COMP)]  # 4个顶点的角度
...
# 四边形的顶点坐标 (π_1)
rotation_angle = 2 * np.pi / 8  # 顺时针旋转 2π/8
...
plt.tight_layout()
# plt.savefig('4_dataset.pdf', format='pdf', bbox_inches='tight')
plt.show()
...
def train_rectified_flow(rectified_flow, optimizer, pairs, batchsize, inner_iters):
  ...
  for i in tqdm(range(inner_iters+1)):
    ...
    num_ones = num_elements // 20  # the number of 1s
...
def draw_plot(rectified_flow, z0, z1, N=None):
  ...
  plt.tight_layout()
  plt.savefig('4_123_output.pdf', format='pdf', bbox_inches='tight') # Should change
