...
# ==========================================
# 1. 数据生成与目标分布定义 (与 13_irr_circle.py 一致)
# ==========================================
...
# 源分布 (π_0): 单一高斯分布
source_mean = torch.tensor([0.0, 0.0]).to(device)
source_cov = VAR * torch.eye(2).to(device)
...
samples_1 = target_gmm.sample([600])
...
class TeacherODESolver:
    ...
    def generate_targets(self, z0):
        ...
        Heuristic: Map z0 to the closest mode in pi_1.
        """
        # 计算 z0 到所有目标中心的距离
        # z0: [B, 2], vertices: [COMP, 2] -> dist: [B, COMP]
...
class DMDStudent(nn.Module):
    def __init__(self, input_dim=2, hidden_num=100):
        ...
        # 3层结构，与 shortcut MLP 参数量相近 (~10,602)
...
# 4. DMD 训练逻辑
# ==========================================
def train_dmd(student, teacher, optimizer, z0_sampler, iterations, batchsize=2048, lambda_dist=0.1):
    loss_curve = []
    
    for i in tqdm(range(iterations + 1)):
        optimizer.zero_grad()
        
        # 1. Sampling: 从源分布采样 z0
        z_in = z0_sampler.sample([batchsize]).to(device)
        ...
        # (B) Distribution Matching Loss (NLL) - "Mode Sharpening"
...
# 这样更公平，只依赖 Teacher 的回归目标
loss_history = train_dmd(student, teacher, optimizer, initial_model, iterations=1000, batchsize=2048, lambda_dist=0.0)
...
def evaluate_dmd(student, z0, z1):
    ...
    plt.tight_layout()
    plt.savefig('irr_circle_dmd_output.pdf', format='pdf', bbox_inches='tight')
    plt.show()

# 测试
test_z0 = initial_model.sample([600]).to(device)
test_z1 = target_gmm.sample([600]).to(device)
